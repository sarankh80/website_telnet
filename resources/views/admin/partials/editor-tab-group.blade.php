...
<div x-data="{
    ...
    init() {
        ...
        console.log('translating =', this.translating);
    },
    ...
    async translate() {
        ...
        try {
            const newTab = await window.adminTranslate(this.$el, this.tab);

            if (newTab) {
                this.tab = newTab;
            } else {
                this.translateError = 'Translation failed. Please try again.';
            }
        } catch (error) {
    ...
    {{-- Header --}}
    <div class="flex flex-wrap items-center gap-2 bg-white px-4 py-2.5 border-b border-slate-800">
        ...
        <button type="button" @click="translate()"
            ...
            <span x-show="!translating" x-cloak>
