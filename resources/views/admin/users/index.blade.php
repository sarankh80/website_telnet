...
<div class="flex items-center justify-between mb-6">
    <p class="text-sm text-slate-400">{{ $users->total() }} users total</p>
...
<form method="GET" class="flex flex-wrap gap-3 mb-6">
    ...
    @if(request()->hasAny(['search','role']))
        <a href="{{ route('admin.users.index') }}" class="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-slate-200 text-sm rounded-lg transition">Clear</a>
    @endif
</form>
    ...
    <table class="w-full text-sm min-w-[640px]">
        <thead>
            <tr class="border-b border-slate-800 text-xs text-slate-400 uppercase tracking-wider">
                ...
                <th class="px-5 py-3.5 text-left hidden md:table-cell">Role</th>
                <th class="px-5 py-3.5 text-center">Status</th>
                <th class="px-5 py-3.5 text-right hidden sm:table-cell">Last Login</th>
                <th class="px-5 py-3.5 text-right hidden sm:table-cell">Joined</th>
